...
test("healthCheck returns structured components payload", async () => {
  const previousGatewayHealthUrl = process.env.OPENCLAW_GATEWAY_HEALTH_URL;
  delete process.env.OPENCLAW_GATEWAY_HEALTH_URL;
  ...
  try {
    ...
    assert.equal(result.components.integrations.gateway.required, false);
    assert.equal(result.components.integrations.gateway.checked, false);
    assert.equal(result.components.integrations.gateway.ok, true);
  } finally {
    if (previousGatewayHealthUrl === undefined) {
      delete process.env.OPENCLAW_GATEWAY_HEALTH_URL;
    } else {
      process.env.OPENCLAW_GATEWAY_HEALTH_URL = previousGatewayHealthUrl;
    }
  }
});
