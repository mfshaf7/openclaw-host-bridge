...
SUPERVISOR="$ROOT/scripts/run-openclaw-host-recovery-supervisor.sh"
...
tmux new-session -d -s "$SESSION_NAME" \
  "/bin/bash -lc 'export OPENCLAW_HOST_BRIDGE_ROOT=\"$ROOT\"; export OPENCLAW_HOST_BRIDGE_CONFIG=\"$CONFIG_PATH\"; export OPENCLAW_CONFIG_PATH=\"$OPENCLAW_CONFIG_PATH_VALUE\"; export OPENCLAW_HOST_RECOVERY_TMUX_SESSION=\"$SESSION_NAME\"; \"$SUPERVISOR\"'"
